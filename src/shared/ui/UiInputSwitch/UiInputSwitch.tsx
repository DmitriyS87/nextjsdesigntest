...
type UiInputSwitchProps = InputProps & GroupProps & {switchPosition?: 'left' | 'right'}

export const UiInputSwitch = ({switchPosition = 'left', addonBefore, addonAfter, children, ...props}: UiInputSwitchProps) => {
    let CompoentBefore = addonBefore;
    let ComponentAfter = addonAfter;

    const isSwitchRight = switchPosition === 'right';

    const ControlledSwitch = () => <Switch size="small" className={cn(styles.switch, {
    ...
    return (
        <Input addonBefore={CompoentBefore} addonAfter={ComponentAfter} {...props} type="button" className={cn(styles.input, {
